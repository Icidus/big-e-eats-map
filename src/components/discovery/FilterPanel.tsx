  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        ...
        <Button type="button" variant="link" size="sm" className="min-h-9 px-0" onClick={onClear}>Clear</Button>
      </div>
...
function FacetGroup({ label, options, selected, onToggle }: { label: string; options: Array<{ id: string; label: string }>; selected: readonly string[]; onToggle(id: string): void }) {
  return <fieldset><legend className="mb-2 font-mono text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">{label}</legend><div className="grid gap-1.5">{options.map((option) => <label key={option.id} className="flex min-h-9 cursor-pointer items-center gap-2 rounded px-1 text-sm hover:bg-secondary/15"><Checkbox checked={selected.includes(option.id)} onCheckedChange={() => onToggle(option.id)} /><span>{option.label}</span></label>)}</div></fieldset>;
}

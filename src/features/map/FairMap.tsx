      ...
      <MapContainer bounds={FAIR_BOUNDS} maxBounds={FAIR_BOUNDS.pad(0.5)} minZoom={15} maxZoom={19} scrollWheelZoom className="h-full w-full">
        ...
        {destination ? (
          <Marker
            ...
            icon={markerIcon(destination.vendor ?? destination.name, destination.locationId ? itemCounts.get(destination.locationId) ?? null : null, ["is-destination", destination.coordinates.precision === "estimated" ? "is-estimated" : ""])}
            zIndexOffset={1000}
            keyboard
          >
            <Popup>
